...
startStep.on("text", async (ctx) => {
    try {
        ...
        await ctx.reply(
            "Какие экзамены по выбору, которые вы сдавали/будете сдавать ",
            kb.examsKb
        );
...
examStep1.action(/exam:(.+)/, async (ctx) => {
    try {
        ...
        ctx.wizard.state.data.exam1 = String(ctx.callbackQuery.data).split(
            ":"
        )[1];
        await ctx.editMessageText(
            `Укажите баллы за экзамен: ${ctx.wizard.state.data.exam1}`
        );
        return ctx.wizard.next();
    } catch (e) {
        await ctx.reply("Произошла ошибка");
...
examStep2.action(/exam:(.+)/, async (ctx) => {
    try {
        // eslint-disable-next-line prefer-destructuring
        ctx.wizard.state.data.exam2 = String(ctx.callbackQuery.data).split(
            ":"
        )[1];
        await ctx.deleteMessage();
        await ctx.reply(
            `Укажите баллы за экзамен: ${ctx.wizard.state.data.exam2}`
        );
        return ctx.wizard.next();
    } catch (e) {
        await ctx.reply("Произошла ошибка");
...
const preExamScene = new Scenes.WizardScene(
    "preExamWizard",
    startStep,
    examStep1,
    examScoreStep1,
